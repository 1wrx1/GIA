...
def grid_plot(tensor, args, ds, dm):
    ...
    file_name = 'saved_img/' + args.arch + '_' + 'cifar100_' + 'bs_' + str(args.bs) + 'act_' + args.act + '.pdf'
...
dataset = datasets.CIFAR100(
        root='/grp01/saas_medml/runxi/cifar100', download=True, train=False,
...
for exp_i in range(exps):
    if args.arch == 'resnet18':
        ...
        model = CustomVitTimm(scale='base', img_size=32, num_classes=100, pretrained=False)
        config['patch'] = args.patch
        #model.reset_classifier(num_classes)
        #model.eval()
    elif args.arch == 'vit-tiny':
        model = CustomVitTimm(scale='tiny', img_size=32, num_classes=100, pretrained=False)
        config['patch'] = args.patch
        #model.eval()
    ...
    # 创建保存图像的目录
    #output_dir = f'output_images/cifar100_bs{size_batch}_' + act + f'/exp_{exp_i}'
