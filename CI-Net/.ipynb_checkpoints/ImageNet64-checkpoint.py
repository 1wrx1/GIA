...
def grid_plot(tensor, args, ds, dm):
    ...
    if 'vit' in args.arch:
        file_name = 'saved_img/' + args.arch + '_' + 'ImageNet64_' + 'bs_' + str(args.bs) + '_act_' + args.act + '_patch_' + str(args.patch) + '.pdf'
    else:
        if args.defense == 'no_defense':
            file_name = 'saved_img/' + args.arch + '_' + 'ImageNet64_' + 'bs_' + str(args.bs) + '_act_' + args.act + '_no_defense_' + '.pdf'
        elif args.defense == 'noise':
            file_name = 'saved_img/' + args.arch + '_' + 'ImageNet64_' + 'bs_' + str(args.bs) + '_act_' + args.act + '_noise_' + str(args.noise) + '.pdf'
        elif args.defense == 'prun':
            file_name = 'saved_img/' + args.arch + '_' + 'ImageNet64_' + 'bs_' + str(args.bs) + '_act_' + args.act + '_prun_' + str(args.p) + '.pdf'
        elif args.defense == 'soteria':
            file_name = 'saved_img/' + args.arch + '_' + 'ImageNet64_' + 'bs_' + str(args.bs) + '_act_' + args.act + '_soteria_' + str(args.p) + '.pdf'
        elif args.defense == 'no_fc':
            file_name = 'saved_img/' + args.arch + '_' + 'ImageNet64_' + 'bs_' + str(args.bs) + '_act_' + args.act + '_no_fc_' + '.pdf'
...
for exp_i in range(exps):
    ...
    if args.arch == 'resnet18':
        ...
        model = CustomVitTimm(scale='base', img_size=64, num_classes=1000, pretrained=False)
        config['patch'] = args.patch
        #model.reset_classifier(num_classes)
        #model.eval()
        config['grad_idx'] = args.grad_idx
    elif args.arch == 'vit-tiny':
        model = CustomVitTimm(scale='tiny', img_size=64, num_classes=1000, pretrained=False)
        config['patch'] = args.patch
        config['grad_idx'] = args.grad_idx
        #model.eval()
    ...
    # 创建保存图像的目录
    '''output_dir = f'output_images/imagenet64_bs{size_batch}_' + act +f'/exp_{exp_i}'
