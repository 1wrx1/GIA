...
class CustomVitTimm(nn.Module):
    def __init__(self, scale='base', img_size=64, num_classes=1000, pretrained=False):
        ...
        nn.init.normal_(self.model.pos_embed, std=0.02)  
...
class train_model(nn.Module):
    def __init__(self, num_classes=100, pretrained=False, arch='resnet18'):
        super(train_model, self).__init__()
        if arch.lower() == 'resnet18':
            self.feature_model = torchvision.models.resnet18(pretrained=pretrained)
        elif arch.lower() == 'resnet34':
            self.feature_model = torchvision.models.resnet34(pretrained=pretrained)
        elif arch.lower() == 'resnet50':
            self.feature_model = torchvision.models.resnet50(pretrained=pretrained)
        elif arch.lower() == 'resnet101':
            self.feature_model = torchvision.models.resnet101(pretrained=pretrained)
        
        self.feature_model.conv1 = nn.Conv2d(in_channels=3, out_channels=64, kernel_size=3, stride=1, padding=1, bias=False)
        self.feature_model.maxpool = nn.Sequential()
        
        self.feature_size = self.feature_model.fc.in_features

        self.feature_model.fc = nn.Sequential()
        
        self.fc = nn.Linear(in_features=self.feature_size, out_features=num_classes, bias=True)  
