...
def _construct_vision_model(cfg_model, cfg_data, pretrained=True, **kwargs):
    ...
    width = cfg_data.shape[1]
    hight = cfg_data.shape[2]
    ...
    if "ImageNet" in cfg_data.name:
        try:
            ...
            if "nfnet" in cfg_model:
            ...
            elif 'resnet18_fc' in cfg_model:
                resnet_model = torchvision.models.resnet18(pretrained=False)
                fc = torch.nn.Linear(resnet_model.fc.in_features, classes)
                #if pretrained:
                    #fc.weight.data = model.fc.weight[:classes]
                    #fc.bias.data = model.fc.bias[:classes]
                resnet_model.fc = fc
                ...
                print(cfg_data.shape)

                print(resnet_model)

                model = nn.Sequential(
                        nn.Flatten(),
                        nn.Linear(channels*width*hight, 1000),
                        nn.Linear(1000, channels*width*hight),
                        nn.Unflatten(dim=1, unflattened_size=(channels, width, hight)),                        
                        resnet_model
                    )
                print('check2')
