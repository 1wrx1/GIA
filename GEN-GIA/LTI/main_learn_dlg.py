...
if args.dataset.lower() == 'cifar10':
    ...
    size = 32
    num_classes = 10
elif args.dataset.lower() == 'imagenet64':
    image_size = 3 * 64 * 64
    size = 64
    num_classes = 1000

if args.shared_model == "LeNet":
    net = LeNet(num_classes, size).to("cuda")
    compress_rate = 1.0
...
if args.trainset == "full":
    checkpoint_name = f"{args.checkpoint_root}/data/{args.dataset}_{args.shared_model}_grad_to_img.pl"
else:
    checkpoint_name = f"{args.checkpoint_root}/data/{args.dataset}_{args.shared_model}_{args.trainset}_grad_to_img.pl"
...
if not os.path.exists(checkpoint_name):
    ...
    if args.dataset == "MNIST":
        ...
        dst_train = datasets.MNIST(f"{args.data_root}", download=True, train=True, transform=transform)
        dst_test = datasets.MNIST(f"{args.data_root}", download=True, train=False, transform=transform)
    elif args.dataset == "CIFAR10":
        transform = transforms.Compose([
                    transforms.ToTensor(),
                    ])

        dst_train = datasets.CIFAR10(f"{args.data_root}/cifar10", download=True, train=True, transform=transform)
        dst_test = datasets.CIFAR10(f"{args.data_root}/cifar10", download=True, train=False, transform=transform)
    elif args.dataset.lower() == "imagenet64":
        transform = transforms.Compose([
        ...
        dst_train = datasets.ImageNet(f"{args.data_root}/ImageNet", split='train', transform=transform)
        dst_test = datasets.ImageNet(f"{args.data_root}/ImageNet", split='val', transform=transform)
    ...
    test_loader = torch.utils.data.DataLoader(dataset=dst_test,
                                                  ...
                                                  shuffle=False)
    print('check')
    def leakage_dataset(data_loader, net, num_classes, length):
        print('check')
        targets = torch.zeros([length, image_size])
        features = torch.zeros([length, model_size])
    ...
    #parallel saving
    checkpoint = {}
    features, targets = leakage_dataset(train_loader, net, num_classes, 50000)
    checkpoint["train_features"] = features
    checkpoint["train_targets"] = targets
    features, targets = leakage_dataset(test_loader, net, num_classes, len(test_loader.dataset))
...
if args.trainset == "full":
    checkpoint_name = f"{args.checkpoint_root}/{args.dataset}_{args.shared_model}_{args.model}_{args.leak_mode}_{args.lr}_{args.epochs}_{args.batch_size}.pt"
else:
    checkpoint_name = f"{args.checkpoint_root}/{args.dataset}_{args.trainset}_{args.shared_model}_{args.model}_{args.leak_mode}_{args.lr}_{args.epochs}_{args.batch_size}.pt"
...
for epoch in tqdm(range(args.epochs)):
    ...
    if (epoch+1)%100 == 0:
        ...
        if args.ood:
            if args.trainset == "full":
                torch.save(checkpoint, f"{args.checkpoint_root}/{args.dataset}_{args.shared_model}_{args.model}_{args.leak_mode}_{args.lr}_{args.epochs}_{args.batch_size}_ood_best.pt")
            else:
                torch.save(checkpoint, f"{args.checkpoint_root}/{args.dataset}_{args.trainset}_{args.shared_model}_{args.model}_{args.leak_mode}_{args.lr}_{args.epochs}_{args.batch_size}_ood.pt")
        else:
            if args.trainset == "full":
                torch.save(checkpoint, f"{args.checkpoint_root}/{args.dataset}_{args.shared_model}_{args.model}_{args.leak_mode}_{args.lr}_{args.epochs}_{args.batch_size}_best.pt")
            else:
                torch.save(checkpoint, f"{args.checkpoint_root}/{args.dataset}_{args.trainset}_{args.shared_model}_{args.model}_{args.leak_mode}_{args.lr}_{args.epochs}_{args.batch_size}.pt")
...
if args.ood:
    if args.trainset == "full":
        torch.save(checkpoint, f"{args.checkpoint_root}/{args.dataset}_{args.shared_model}_{args.model}_{args.leak_mode}_{args.lr}_{args.epochs}_{args.batch_size}_ood.pt")
    else:
        torch.save(checkpoint, f"{args.checkpoint_root}/{args.dataset}_{args.trainset}_{args.shared_model}_{args.model}_{args.leak_mode}_{args.lr}_{args.epochs}_{args.batch_size}_ood.pt")
else:
    if args.trainset == "full":
        torch.save(checkpoint, f"{args.checkpoint_root}/{args.dataset}_{args.shared_model}_{args.model}_{args.leak_mode}_{args.lr}_{args.epochs}_{args.batch_size}.pt")
    else:
        torch.save(checkpoint, f"{args.checkpoint_root}/{args.dataset}_{args.trainset}_{args.shared_model}_{args.model}_{args.leak_mode}_{args.lr}_{args.epochs}_{args.batch_size}.pt")
